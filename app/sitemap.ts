...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const silverPriceEntries = generateEntries("/silver-price", new Date(), "hourly", 0.9)

  const mainPages = ["/catalog", "/collections", "/workshop"].flatMap((route) =>
    generateEntries(route, new Date(), "weekly", 0.8)
  )

  const supportPages = ["/about", "/contact", "/store-location"].flatMap((route) =>
    generateEntries(route, new Date(), "monthly", 0.7)
  )
  ...
  const collectionEntries = collections.flatMap((collection) =>
    generateEntries(`/collections/${collection.slug}`, new Date(), "weekly", 0.7)
  )

  const jewelryEntries = jewelry.flatMap((item) =>
    generateEntries(`/product/${item.slug}`, new Date(item.updated_at), "weekly", 0.8)
  )
