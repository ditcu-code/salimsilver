...
export async function updateJewelry(id: string, formData: FormData) {
  ...
  const { error } = await supabase.from("jewelry").update(rawData).eq("id", id)
...
export async function saveJewelryImage(jewelryId: string, imagePath: string) {
  ...
  const {
    data: { publicUrl },
  } = supabase.storage.from("catalog").getPublicUrl(imagePath)

  const { error } = await supabase.from("jewelry_images").insert({
...
export async function deleteJewelryImage(imageId: string) {
  ...
  if (image?.src) {
    ...
    if (parts.length === 2) {
      ...
      const { error: storageError } = await supabase.storage.from("catalog").remove([storagePath])
...
export async function createCollection(formData: FormData) {
  ...
  const { data, error } = await supabase.from("collections").insert(rawData).select("id").single()
...
export async function updateCollection(id: string, formData: FormData) {
  ...
  const { error } = await supabase.from("collections").update(rawData).eq("id", id)
